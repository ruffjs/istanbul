...
function filesFor(options, callback) {
    ...
    excludes = excludes && Array.isArray(excludes) ? excludes : [ '**/node_modules/**', '**/ruff_modules/**' ];
    ...
    try {
        // Asynchronous glob leads to random crash, might be issue with
        // parallel FS operations of Ruff.
        files = glob.sync(includes.join(' '), opts);
    } catch (err) {
        return setImmediate(callback, err);
    }

    if (relative) {
        return setImmediate(callback, null, files);
    }
